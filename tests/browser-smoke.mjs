...
await new Promise(resolve=>setTimeout(resolve,200));
text=await evaluate('document.body.innerText');
assert(text.includes('One skill for all 12 questions'),'daily lesson did not keep one skill focus');
assert(text.includes('Worked example:'),'daily lesson teaching step missing');
console.log('browser learning-app smoke: PASS (dashboard, assessment, results, persistence, one-skill teaching lesson, desktop/mobile fit)');
ws.close();
